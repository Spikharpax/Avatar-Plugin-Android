...
// consts
	...
	const TTSWAV = join(__dirname, "..", "batchs", "ttswav.vbs");
		...
		// standardise
			...
			const result = [];
				...
				voices.forEach((_voice) => {
					...
					const voice = {
						"gender": IS_SAPI ? _voice.Gender ? _voice.Gender.trim().toLowerCase() : "" : "",
			...
			if ("undefined" === typeof options.text) {
				...
				this._writePromise = Promise.resolve().then(() => {
					...
					return this._forceStop ? Promise.resolve(options) : Promise.resolve().then(() => {
						...
						const args = readOptionsToSAPIArgs(options);
						...
						return new Promise((resolve, reject) => {
							this._writing = spawn("cscript", CSCRIPT_ARGS.concat([ TTSWAV ]).concat(args));

							let err = "";
							this._writing.stderr.on("data", (data) => {
								err += "string" === typeof data ? data : data.toString("ascii");
							});

							this._writing.on("close", (code) => {
